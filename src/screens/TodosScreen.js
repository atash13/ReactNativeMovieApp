...
export default function TodosScreen() {
  ...
  const storeObjectData = async value => {
    try {
      await AsyncStorage.setItem('my-key-object', JSON.stringify(value));
    } catch (error) {}
  };

  const getDataObject = async (key = 'my-key-object') => {
    try {
      const value = await AsyncStorage.getItem(key);
      console.warn('first', JSON.parse(value));
  ...
  const mergeData = async (key = 'my-key') => {
    try {
      await AsyncStorage.mergeItem(key, 'React Native Storage');
    } catch (error) {}
  };

  const mergeObjectData = async (key = 'my-key-object') => {
    try {
      const value = await getDataObject();
  ...
  const removeData = async (key = 'my-key') => {
    try {
  ...
  return (
    <SafeAreaView className="flex-1 items-center">
      ...
      <Button
        title="Store Data"
        onPress={async () =>
          await storeData('Furkan Türkyılmaz ile React Native Eğitimi')
        }
      />
      <Button
        title="Store Array Data"
        onPress={async () => await storeObjectData(todos)}
      />
      <Button title="Get Data" onPress={async () => await getData()} />
      <Button
        title="Get Data Object"
        onPress={async () => await getDataObject()}
      />
      <Button title="Delete Data" onPress={async () => await removeData()} />
      <Button title="Merge Data" onPress={async () => await mergeData()} />
      <Button
        title="Merge Data Object"
        onPress={async () => await mergeObjectData()}
      />
    </SafeAreaView>
  );
}